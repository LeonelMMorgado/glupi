...
Camera *camera_create(Window *win, CameraType type, Camera_Args args) {
    ...
    if(type == ORTHOGRAPHIC_CAMERA) {
		...
		c->camera_p.sensitivity = vec2_scalar_mul(vec2_one(), 0.1);
		c->camera_p.yaw = -90.0;
        c->camera_p.fov = args.p_fov;
        c->camera_p.aspect = (float)win->settings.size.x / (float)win->settings.size.y;
        ...
        c->view_proj.proj = mat4_perspective(c->camera_p.fov * M_PI / 180.0, c->camera_p.aspect, c->camera_p.zNear, c->camera_p.zFar);
...
void camera_update_proj(Camera *camera) {
    ...
    if(camera->type == PERSPECTIVE_CAMERA)
        camera->view_proj.proj = mat4_perspective(camera->camera_p.fov * M_PI / 180.0, camera->camera_p.aspect, camera->camera_p.zNear, camera->camera_p.zFar);
}
